...
function CcZnp () {
    ...
    this._innerListeners = {
        ...
        spErr: function (err) {
            debug("An error occured with the serialport: %s", err)
            if(self._sp.isOpen) self._sp.close();
        },
        spClose: function () {
...
CcZnp.prototype.close = function (callback) {
    ...
    if (this._init) {
        this._sp.flush(function () {
            if(self._sp.isOpen) self._sp.close(callback);
        });
    } else {
        callback(null);
    }
...
CcZnp.prototype.request = function (subsys, cmd, valObj, callback) {
    ...
    if (argObj.type === 'SREQ') {
        logSreq('--> %s, %o', argObj.subsys + ':' + argObj.cmd, valObjFormat(valObj));
        return this._sendSREQ(argObj, callback);
    } else if (argObj.type === 'AREQ') {
        logAreq('--> %s, %o', argObj.subsys + ':' + argObj.cmd, valObjFormat(valObj));
...
CcZnp.prototype._mtIncomingDataHdlr = function (err, data) {
    ...
    if (data.type === 'SRSP') {
        logSrsp('<-- %s, %o', subsys + ':' + cmd, valObjFormat(result));
        rxEvt = 'SRSP:' + subsys + ':' + cmd;
        this.emit(rxEvt, result);
    } else if (data.type === 'AREQ') {
        logAreq('<-- %s, %o', subsys + ':' + cmd, valObjFormat(result));
        rxEvt = 'AREQ';
        msg = {
            subsys: subsys,
            ind: cmd,
